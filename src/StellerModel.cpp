...
StellerModel::StellerModel(/* args */)
{
}

StellerModel::~StellerModel()
{
}

void StellerModel::Calc(){
	...
	bool converge = false;
	while(!converge){
		steller.SetOuterBoundary();
		steller.SetInnerBoundary();
		Phys physMidI=steller.ShootIn();//OK
		Phys physMidO=steller.ShootOut();//OK
		// physMidI.Out();
		// physMidO.Out();
		converge = CheckConverge(physMidI,physMidO);
		steller.SetPerturbPc();
		physMidOp1=steller.ShootOut();
		steller.SetPerturbTc();
		physMidOp2=steller.ShootOut();
		steller.SetPerturbLs();
		physMidIp1=steller.ShootIn();
		steller.SetPerturbRs();
		physMidIp2=steller.ShootIn();
		// physMidOp1.Out();	
		// physMidOp2.Out();	
		// physMidIp1.Out();	
		// physMidIp2.Out();	
		...
		d_deltaR_dPc = phys_dPc.getR() / (0.01*steller.getPc());
		d_deltaR_dTc = phys_dTc.getR() / (0.01*steller.getTc());
		d_deltaR_dLs = phys_dLs.getR() / (0.01*steller.getLs());
		d_deltaR_dRs = phys_dRs.getR() / (0.01*steller.getRs());

		d_deltaP_dPc = phys_dPc.getP() / (0.01*steller.getPc());
		d_deltaP_dTc = phys_dTc.getP() / (0.01*steller.getTc());
		d_deltaP_dLs = phys_dLs.getP() / (0.01*steller.getLs());
		d_deltaP_dRs = phys_dRs.getP() / (0.01*steller.getRs());

		d_deltaT_dPc = phys_dPc.getT() / (0.01*steller.getPc());
		d_deltaT_dTc = phys_dTc.getT() / (0.01*steller.getTc());
		d_deltaT_dLs = phys_dLs.getT() / (0.01*steller.getLs());
		d_deltaT_dRs = phys_dRs.getT() / (0.01*steller.getRs());

		d_deltaL_dPc = phys_dPc.getL() / (0.01*steller.getPc());
		d_deltaL_dTc = phys_dTc.getL() / (0.01*steller.getTc());
		d_deltaL_dLs = phys_dLs.getL() / (0.01*steller.getLs());
		d_deltaL_dRs = phys_dRs.getL() / (0.01*steller.getRs());
		...
		steller.setPc(steller.getPc()-x[0]);
		steller.setTc(steller.getTc()-x[1]);
		steller.setLs(steller.getLs()-x[2]);
		steller.setRs(steller.getRs()-x[3]);
	}

	Phys physSurface = steller.getPhys(Ndim-2);
	Phys physCenter  = steller.getPhys(0);
	physSurface.Out();
	physCenter.Out();
}

/*
void StellerModel::Iterate(){
	// bool converge = false;
	// steller1->SetInnerBoundary();
	// steller2->SetOuterBoundary();
	// while(!converge){
	// 	steller1->ShootOut();
	// 	steller2->ShootIn();
	// 	converge = CheckConverge(steller1,steller2);
	// 	steller1p->SetPerturbPc();
	// 	steller1p->ShootOut();
	// 	steller1p->SetPerturbTc();
	// 	steller1p->ShootOut();
	// 	steller2p->SetPerturbLs();
	// 	steller2p->ShootIn();
	// 	steller2p->SetPerturbRs();
	// 	steller2p->ShootIn();
	// }
	// Plot();
	...
	int itr = 0;
	steller1->SetInnerBoundary();
	steller1->SetOuterBoundary();
	while(!converge){
		steller1->ShootOut();
		steller1->ShootIn();
		converge = steller1->CheckConverge(itr);
		steller1->SetPerturbPc();
		steller1->ShootOut();
		steller1->SetPerturbTc();
		steller1->ShootOut();
		//
		steller1->SetPerturbLs();
		steller1->ShootIn();
		steller1->SetPerturbRs();
		steller1->ShootIn();
		//
		itr++;
	}
...
void StellerModel::Plot(){
}

Steller::Steller():logOut(false){
	double mstar_fact = 1;
	X=0.70;
	Y=0.28;
	Z=0.02;
...
Steller::Steller(double mstarFact):logOut(false){
	X=0.70;
	Y=0.28;
	Z=0.02;
	Setup(mstarFact,X,Y,Z);
}
Steller::Steller(double mstarFact,double x,double y, double z):logOut(false){
	Setup(mstarFact,x,y,z);
}

void Steller::Setup(double mstarFact,double x,double y, double z){
...
double Steller::getPc(){return Pc;}
double Steller::getTc(){return Tc;}
double Steller::getPs(){return Ps;}
double Steller::getTs(){return Ts;}
double Steller::getRs(){return Rs;}
double Steller::getLs(){return Ls;}

Phys Steller::getPhys(int index){
	return Phys(M[index],R[index],P[index],T[index],L[index],rho[index],kappa[index],epsilon[index]);
}

void Steller::setPc(double pc){Pc=pc;}
void Steller::setTc(double tc){Tc=tc;}
void Steller::setLs(double ls){Ls=ls;}
void Steller::setRs(double rs){Rs=rs;}



void Steller::SetInnerBoundary(){
	P[0]=Pc;
	T[0]=Tc;
...
bool StellerModel::CheckConverge(Phys phys1,Phys phys2){
...
bool Steller::CheckConverge(Phys phys1,Phys phys2){
	double Rdiff, Pdiff,Tdiff,Ldiff;
	double Rmidpoint, Pmidpoint,Tmidpoint,Lmidpoint;
	Rdiff = phys1.getR() - phys2.getR();
	Pdiff = phys1.getP() - phys2.getP();
...
void Steller::SetLogOut(bool sw){logOut=sw;}
