...
StellarModel::StellarModel(/* args */)
{
}
...
void StellarModel::Calc(){
	...
	bool converge = false;
	while(!converge){
		stellar.SetOuterBoundary();
		stellar.SetInnerBoundary();
		Phys physMidI=stellar.ShootIn();//OK
		Phys physMidO=stellar.ShootOut();//OK
		// physMidI.Out();
		// physMidO.Out();
		converge = CheckConverge(physMidI,physMidO);
		stellar.SetPerturbPc();
		physMidOp1=stellar.ShootOut();
		stellar.SetPerturbTc();
		physMidOp2=stellar.ShootOut();
		stellar.SetPerturbLs();
		physMidIp1=stellar.ShootIn();
		stellar.SetPerturbRs();
		physMidIp2=stellar.ShootIn();
		// physMidOp1.Out();	
		// physMidOp2.Out();	
		// physMidIp1.Out();	
		// physMidIp2.Out();	
		...
		d_deltaR_dPc = phys_dPc.getR() / (0.01*stellar.getPc());
		d_deltaR_dTc = phys_dTc.getR() / (0.01*stellar.getTc());
		d_deltaR_dLs = phys_dLs.getR() / (0.01*stellar.getLs());
		d_deltaR_dRs = phys_dRs.getR() / (0.01*stellar.getRs());

		d_deltaP_dPc = phys_dPc.getP() / (0.01*stellar.getPc());
		d_deltaP_dTc = phys_dTc.getP() / (0.01*stellar.getTc());
		d_deltaP_dLs = phys_dLs.getP() / (0.01*stellar.getLs());
		d_deltaP_dRs = phys_dRs.getP() / (0.01*stellar.getRs());

		d_deltaT_dPc = phys_dPc.getT() / (0.01*stellar.getPc());
		d_deltaT_dTc = phys_dTc.getT() / (0.01*stellar.getTc());
		d_deltaT_dLs = phys_dLs.getT() / (0.01*stellar.getLs());
		d_deltaT_dRs = phys_dRs.getT() / (0.01*stellar.getRs());

		d_deltaL_dPc = phys_dPc.getL() / (0.01*stellar.getPc());
		d_deltaL_dTc = phys_dTc.getL() / (0.01*stellar.getTc());
		d_deltaL_dLs = phys_dLs.getL() / (0.01*stellar.getLs());
		d_deltaL_dRs = phys_dRs.getL() / (0.01*stellar.getRs());
		...
		stellar.setPc(stellar.getPc()-x[0]);
		stellar.setTc(stellar.getTc()-x[1]);
		stellar.setLs(stellar.getLs()-x[2]);
		stellar.setRs(stellar.getRs()-x[3]);
	}

	Phys physSurface = stellar.getPhys(Ndim-2);
	Phys physCenter  = stellar.getPhys(0);
	physSurface.Out();
	physCenter.Out();
}
...
bool StellarModel::CheckConverge(Phys phys1,Phys phys2){
